...
import kotlinx.android.synthetic.main.activity_payment_home.*
...
class PaymentHomeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        total_amt_payable.text = intent.extras!!.getString("amt")!!
    ...
    private fun useCODInApp() {
        val cod = HashMap<String, Any>()
        cod["paymentmode"] = "COD"
        val ref1 = fStore.collection("HotBox").document(userid).collection("HomeOrders")
        ref1.addSnapshotListener { snapshot, e ->
            for(ds in snapshot!!.documents){
                fStore.collection("HotBox").document(userid).collection("HomeOrders").document(ds.id).update(cod)
